...
class WhatsappConversation {
    ...
    receiveRequest(req, res) {
        ...
        if (this.purecloud == false) {
            firestoreConnector.insertDialog(this.senderId, 'Client', 'Bot', req.body.Body, new Date());
        }
        else {
            firestoreConnector.insertDialog(this.senderId, 'Client', 'PureCloud', req.body.Body, new Date());
        }
    ...
    sendResponseToClient(text, source) {
        firestoreConnector.insertDialog(this.senderId, source, 'Client', text, new Date());
