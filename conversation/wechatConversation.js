...
class WechatConversation {
    ...
    receiveRequest(data, res) {
        ...
        if (this.purecloud == false) {
            firestoreConnector.insertDialog(this.senderId, 'Client', 'Bot', data.Content, new Date());
        }
        else {
            firestoreConnector.insertDialog(this.senderId, 'Client', 'PureCloud', data.Content, new Date());
        }
